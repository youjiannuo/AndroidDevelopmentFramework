...
public class WebViewUtil {
    ...
    public void setUrl(final String url, String data, final Map<String, String> httpHeaders, final int type, final String params) {
        setUrl(addToken(url), data, httpHeaders, type, params, false);
    }

    private String addToken(String url) {
        if (url.contains("?")) {
            url += "&token=" + UserSharePreferences.getToken();
        } else {
            url += "?token=" + UserSharePreferences.getToken();
        }
        return url;
    ...
    public void setUrl(final String url, final String data, final Map<String, String> httpHeaders, final int type, final String params, boolean isLoad) {
        ...
        mWebView.setWebViewClient(new WebViewClient() {
            ...
            public void onReceivedSslError(WebView view, SslErrorHandler handler, SslError error) {
                SystemUtil.printlnInfo(error.toString());
                SslCertificate ssl = error.getCertificate();
                Bundle bundle = SslCertificate.saveState(ssl);
                byte[] bytes = bundle.getByteArray("x509-certificate");
                CertificateFactory cf;
                try {
                    cf = CertificateFactory.getInstance("X.509", "BC");
                    Certificate ca = cf.generateCertificate(new ByteArrayInputStream(bytes));
                    Certificate newCert = initPrivateKeyAndX509Certificate();
                    if (Arrays.equals(ca.getEncoded(), newCert.getEncoded())) {
                        handler.proceed();
                        return;
                    }
                } catch (Exception e) {
            ...
            public void run() {
                try {
                    ...
                    if (httpHeaders != null) {
                        if (mWebView != null) {
                            if (type == GET) {
                                mWebView.loadUrl(url.trim(), httpHeaders);
                            } else {
                                mWebView.postUrl(url.trim(), params.getBytes());
                            }
                        }
                    } else {
                        if (mWebView != null) {
    ...
    public void onDestroy() {
        try {
            if (mWebView != null) {
                ...
                //清空所有Cookie
                CookieSyncManager.createInstance(ContextManager.getContext());  //Create a singleton CookieSyncManager within a context
    ...
    private X509Certificate initPrivateKeyAndX509Certificate()
            throws Exception {
        AssetManager assetManager = ContextManager.getContext().getAssets();
        InputStream inputStream = assetManager.open(ContextManager.getString(R.string.yn_http_public_key));
