...
public enum TaskQueue {
    ...
    private void removeTask(String key, ImageViewNetwork v) {
        ...
        Util.println("剩余任务:key:" + imageViewNetworks.size());
    }
    ...
    public void pauseAllTask(String key) {
        ...
        for (Object object : objects) {
            //关闭任务
            ((ImageViewNetwork) object).stopTask();
            Util.println("key = " + key + "   结束任务:" + ((ImageViewNetwork) object).getTask().url);
        }
        Util.println("key = " + key + "已经结束任务:" + objects.length);
    }
    ...
    public void resumeAllTask(String key) {
        ...
        for (Object object : objects) {
            ((ImageViewNetwork) object).stopTask();
            ((ImageViewNetwork) object).startTask();
            Util.println("key = " + key + "重新启动任务:" + ((ImageViewNetwork) object).getTask().url);
        }
        Util.println("key = " + key + "已经开启:" + objects.length);
    }
