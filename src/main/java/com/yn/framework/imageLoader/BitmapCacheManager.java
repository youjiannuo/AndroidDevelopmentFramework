...
import static com.yn.framework.imageLoader.NetworkPhotoTask.FILE_IMG;
...
public enum BitmapCacheManager {
    ...
    public Bitmap getBitmapFromCacheAndNative(NetworkPhotoTask task) {
        ...
        if (bitmap == null) {
            ...
            //是否需要进行高斯模糊处理
            if (task.isGaussianBlur) {
                //设置高斯模糊
                bitmap = ImageUtil.getBlurFilter(bitmap);
            }
        }
        return bitmap;
    }
    ...
    public Bitmap getNativeBitmap(NetworkPhotoTask task) {
        ...
        Bitmap bitmap;
        try {
            bitmap = BitmapFactory.decodeFile(task.getPhotoName(), options);
