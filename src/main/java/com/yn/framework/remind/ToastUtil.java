...
public class ToastUtil {
    ...
    public static void showNormalMessage(String s) {
        showNormalMessage(s, Toast.LENGTH_SHORT);
    }

    public static void showNormalMessage(String s, int length) {
        showNormalMessage(s, length, false);
    }

    //show normal message
    public static void showNormalMessage(String s, int length, boolean isCenter) {
        if (s == null || s.trim().length() == 0) return;
        final long t = new Date().getTime();
        if (text.equals(s) && t - time < 2000) {
            //如果小于2秒 就不会有提醒
...
//        toast.show();
        try {
            Toast toast = Toast.makeText(ContextManager.getContext(), s, length);
            toast.setGravity(Gravity.CENTER, 0, 0);
            if (isCenter) {
