...
import static com.yn.framework.system.SystemUtil.setTranslucentStatus;
...
public class YNCommonActivity extends FragmentActivity implements View.OnClickListener,
    ...
    public static final int BACK_FINISH = 0x888;
...
//            mBarView.setRightImageButton(drawable);
    ...
    public void setVisibleView(int visible, List<View> views) {
        for (View view : views) {
            view.setVisibility(visible);
        }
    }

    public void setVisibleView(int visible, View... views) {
        for (View view : views) {
            view.setVisibility(visible);
        }
    }
...
//        setStatusBarColor();
    ...
    public void showRemindBox(String[] button, String message, String title, int type) {
...
//    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null) {
            if (resultCode == BACK_FINISH) {
                String stopClassArr[] = data.getStringArrayExtra("stopClass");
                if (!isClassIn(stopClassArr)) {
                    setResult(BACK_FINISH, data);
                    finish();
                }
            }
        }
    ...
    private boolean isClassIn(String[] stopClassArray) {
        for (String aStopClassArray : stopClassArray) {
            if (getClass().toString().equals(aStopClassArray)) {
                return true;
            }
        }
        return false;
...
//            }.start();
    ...
    public boolean checkParams(View view, int position) {
        return false;
    }

    public String[] getHttpValue(View view, int position) {
        return new String[0];
    }

    public String[] getHttpKey(View view, int position) {
        return new String[0];
    }
    ...
    public Object[] getTitleAndMsgValue(View view, int position) {
        return new Object[0];
    }

    public String[] getButtonString(View view, int position) {
        return new String[0];
    }
    ...
    protected void backActivity(Intent intent, Class... cls) {
        String clsArr[] = new String[cls.length];
        for (int i = 0; i < cls.length; i++) {
            clsArr[i] = cls[i].toString();
        }
        intent.putExtra("stopClass", clsArr);
        setResult(BACK_FINISH, intent);
        finish();
    }

    protected void backActivity(Class cls, Intent intent) {
        backActivity(intent, cls);
    }
