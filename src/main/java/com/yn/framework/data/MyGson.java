...
public class MyGson {
    ...
    private Object getObject(Class<?> t, String data, List<Class> cls, List<Class> replaceCls) {
        ...
        for (Class cl = t; cl != Object.class && cl != null; cl = cl.getSuperclass()) {
                ...
                try {
                    ...
                    if (value == null) {
                        if (field.getType() == List.class || field.getType() == ArrayList.class) {
                            value = getList(field, json.getString(getFiledName(field)));
                        } else {
                            ...
                            value = getObject(c, json.getString(getFiledName(field)), cls, replaceCls);
                        }
                    }
                    if (value != null) {
                        field.set(obj, value);
    ...
    public  <T> List<T> getList(Type type, String data) {
        ...
        for (int i = 0 ; i < json.size() ; i ++){
            T t = (T) getObject(cls, json.getArrayIndex(i), null, null);
            //修改了
            if (t != null) {
                list.add(t);
            }
    ...
    public <T> List<T> getList(Class cls, String data) {
        ...
        JSON json = new JSON(data);
        while (json.next()) {
            T t = (T) getObject(cls, json.getItemString(), null, null);
            //修改了
            if (t != null) {
                list.add(t);
            }
